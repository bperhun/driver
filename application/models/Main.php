...
class Main extends Model {
	...
	public function driversCount() {
		...
		return $this->db->column('SELECT COUNT(id) FROM accountss');
	}

	public function driversList($route) {
		$max = 10;
		$params = [
			'max' => $max,
			'start' => ((($route['page'] ?? 1) - 1) * $max),
		];
		return $this->db->row('SELECT * FROM accountss ORDER BY id DESC LIMIT :start, :max', $params);
	}
