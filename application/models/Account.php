...
class Account extends Model {
	...
	public function validate($input, $post) {
		$rules = [
			...
			'name' => [
				'pattern' => '#^[a-z0-9]{3,20}$#',
				'message' => 'Имя указано неверно (разрешены только латинские буквы от 3 до 20 символов',
			],
			'town' => [
				'pattern' => '#^[a-z0-9]{3,15}$#',
				'message' => 'Город указан неверно (разрешены только латинские буквы от 3 до 15 символов',
			],
			'phone' => [
				'pattern' => '#^[a-z0-9]{9,11}$#',
				'message' => 'Номер указан неверно (разрешены только цифры от 9 до 11 символов',
			],
		];
	...
	public function checkEmailExists($email) {
		...
		return $this->db->column('SELECT id FROM accountss WHERE email = :email', $params);
	}
	...
	public function checkLoginExists($login) {
		...
		if ($this->db->column('SELECT id FROM accountss WHERE login = :login', $params)) {
	...
	public function register($post) {
		...
		$this->db->query('INSERT INTO accountss VALUES (:id, :email, :login, :password, :name, :town, :phone)', $params);
	...
	public function checkData($login, $password) {
		...
		$hash = $this->db->column('SELECT password FROM accountss WHERE login = :login', $params);
	...
	public function login($login) {
		...
		$data = $this->db->row('SELECT * FROM accountss WHERE login = :login', $params);
	...
	public function save($post) {
		...
		$this->db->query('UPDATE accountss SET email = :email, password = :password, name = :name, town = :town, phone = :phone'.$sql.' WHERE id = :id', $params);
	}
